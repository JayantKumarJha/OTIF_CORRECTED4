# -*- coding: utf-8 -*-
"""Costumer_OTIF4.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1MWZY-fif2WbyCYNd4NJo8gsmWds9VTFl
"""

# pages/Customer_OTIF.py
# -------------------------------------------------------------
# Customer OTIF Analysis — Streamlit
# - Required columns (with dots): COUNTRY, GENERIC NAME, ORDER NO,
...
# - Month bucketing = last ACTUAL DISPATCH DATE per order
# - Yearly OTIF KPI + monthly chart/table
# - Country-wise Customer Contribution % and Customer OTIF % (for ALL countries)
# - Top countries with breaches table includes: Failures, Total, Contrib %, Country OTIF %
# - PDF export lists ALL failed orders by Country with those metrics in header
# -------------------------------------------------------------
...
st.caption(
    ...
    "(an order fails if ANY line is late or not in full), then show monthly and country performance. "
...
def to_float_series(s: pd.Series) -> pd.Series:
    """Robust float conversion: removes commas/spaces, converts (1,234) -> -1234, handles stray text."""
...
def to_datetime_utc(s: pd.Series) -> pd.Series:
    """Parse to timezone-aware UTC datetimes (prevents naive/aware comparison errors)."""
...
def load_and_clean(file) -> pd.DataFrame:
    ...
    # Also guard against empty-string in key text columns
    ...
    lines = (
        df.groupby(["ORDER NO", "GENERIC NAME"], as_index=False)
          .agg({
              ...
              "COUNTRY": "first",  # keep for grouping later
          })
    )
...
def build_country_stats(orders_year: pd.DataFrame) -> pd.DataFrame:
    """
    Country-wise stats for selected year:
    - Total_Orders_Country
    - OTIF_Success
    - OTIF_Failures
    - Country_Contribution_% (of total orders in year)
    - Country_OTIF_% (success/total for country)
    """
    total_orders_year = float(len(orders_year))
    by_country = (orders_year.groupby("Country")
                            .agg(Total_Orders_Country=("ORDER NO", "count"),
                                 OTIF_Success=("OTIF", "sum"))
                            .reset_index())
    by_country["OTIF_Failures"] = by_country["Total_Orders_Country"] - by_country["OTIF_Success"]
    by_country["Country_Contribution_%"] = (by_country["Total_Orders_Country"] / total_orders_year) * 100.0
    by_country["Country_OTIF_%"] = (by_country["OTIF_Success"] / by_country["Total_Orders_Country"].replace(0, np.nan)) * 100.0
    by_country["Country_OTIF_%"] = by_country["Country_OTIF_%"].fillna(0.0)
    return by_country

def generate_failed_orders_pdf(breaches_df: pd.DataFrame, country_stats_df: pd.DataFrame, year: int) -> bytes:
    """
    Create a PDF listing ALL failed OTIF orders grouped by COUNTRY, descending by failures.
    Country header shows: Failures, Total, Contrib %, Country OTIF %.
    Each order line: 'dd-mm-YYYY    ORDER NO'
    """
    if not REPORTLAB_OK:
        raise RuntimeError("reportlab not available")

    # Counts for failures & merge with stats for headers
    fail_counts = (breaches_df.groupby("Country")
                               .size()
                               .reset_index(name="OTIF_Failures"))
    header_df = (fail_counts.merge(country_stats_df, on="Country", how="left")
                           .sort_values("OTIF_Failures", ascending=False))
    ...
    for _, row in header_df.iterrows():
        country = row["Country"]
        failures = int(row.get("OTIF_Failures", 0))
        total_orders = int(row.get("Total_Orders_Country", 0))
        contrib = float(row.get("Country_Contribution_%", 0.0))
        otif_pct = float(row.get("Country_OTIF_%", 0.0))
        ...
        header_text = (f"{idx}. {country}  "
                       f"(Failures: {failures} | Total: {total_orders} | "
                       f"Contrib: {contrib:.1f}% | Country OTIF: {otif_pct:.1f}%)")
        c.drawString(margin_x, y, header_text)
        y -= lh
...
# Country stats (for ALL countries)
country_stats = build_country_stats(orders_year)
...
# KPIs — yearly order-level OTIF + total orders
k1, k2 = st.columns(2)
...
# --------------------------- Countries with OTIF Breaches (+ Contribution & OTIF%) ---------------------------
...
if breaches.empty:
    ...
    # Still show full country stats for transparency
    all_c_display = country_stats.copy()
    for c in ["Country_Contribution_%", "Country_OTIF_%"]:
        all_c_display[c] = all_c_display[c].astype(float).round(1)
    all_c_display = all_c_display.sort_values("Total_Orders_Country", ascending=False)
    st.dataframe(
        all_c_display[["Country", "Total_Orders_Country", "Country_Contribution_%", "Country_OTIF_%"]],
        use_container_width=True
    )
else:
    # Breach counts
    fail_counts = (breaches.groupby("Country")
                             .size()
                             .reset_index(name="OTIF_Breaches"))
    # Merge with country stats
    top_countries = (fail_counts.merge(country_stats, on="Country", how="left")
                               .sort_values("OTIF_Breaches", ascending=False)
                               .head(10))

    # Pretty formatting
    top_display = top_countries.copy()
    for c in ["Country_Contribution_%", "Country_OTIF_%"]:
        top_display[c] = top_display[c].astype(float).round(1)
    cols = ["Country", "OTIF_Breaches", "Total_Orders_Country", "Country_Contribution_%", "Country_OTIF_%"]
    st.dataframe(top_display[cols], use_container_width=True)

    # PDF of ALL failed orders grouped by country (with metrics in header)
    if REPORTLAB_OK:
        try:
            pdf_bytes = generate_failed_orders_pdf(
                breaches[["ORDER NO", "Country", "Last_Actual_Dispatch"]],
                country_stats[["Country", "Total_Orders_Country", "OTIF_Success", "OTIF_Failures", "Country_Contribution_%", "Country_OTIF_%"]],
                selected_year
            )
            st.download_button(
            ...
            csv_bytes = breaches[["Country", "Last_Actual_Dispatch", "ORDER NO"]].sort_values(
                ["Country", "Last_Actual_Dispatch"], ascending=[False, False]
...
# --------------------------- All Countries table ---------------------------
with st.expander("🌍 All Countries — Contribution & OTIF%"):
    all_c = country_stats.copy()
    for c in ["Country_Contribution_%", "Country_OTIF_%"]:
        all_c[c] = all_c[c].astype(float).round(1)
    all_c = all_c.sort_values("Total_Orders_Country", ascending=False)
    st.dataframe(
        all_c[["Country", "Total_Orders_Country", "OTIF_Success", "OTIF_Failures", "Country_Contribution_%", "Country_OTIF_%"]],
        use_container_width=True
    )
...
with st.expander("🔎 Debug / Sanity"):
    ...
    st.write("Country stats (first 10):", country_stats.head(10))