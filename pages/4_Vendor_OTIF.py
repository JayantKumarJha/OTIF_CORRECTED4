...
# Updated Streamlit OTIF analysis — column names preserve trailing full stops (dots)
...
# ---------- IMPORTANT: required columns now include full stops (dots) ----------
REQUIRED_COLS = [
...
def clean_columns(df: pd.DataFrame) -> pd.DataFrame:
    """
    Clean column names but DO NOT strip periods — we want trailing full stops preserved.
    """
    df = df.copy()
    df.columns = (
        df.columns
        ...
        # NOTE: intentionally do NOT strip '.' characters so columns like 'P.O. Dt.' remain intact
        .str.strip()
    )
...
def ensure_types_and_drop_nulls(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    # NOTE: column names include dots now
...
def compute_po_level_metrics(df: pd.DataFrame):
    # Product-level fulfillment (sum duplicates) — use dotted column names
    df_po_item = (
    ...
    # PO-level On-Time (use last GRN per PO for bucketing later)
...
def generate_failed_orders_pdf(breaches_df: pd.DataFrame, vendor_stats: pd.DataFrame, year: int) -> bytes:
    ...
    PDF heading includes OTIF % and Total Contribution % for each vendor.
    """
    ...
    # vendors ordered by failures desc
...
if not st.session_state.get("data_file_loaded"):
    st.info(
        "Upload your Excel to begin. Expected columns: "
        "Mat Type, Material Code, Material Name, UOM, P.O. Dt., P. O. No., Supplier, PO Qty., GNR Dt., Inward Qty."
    )
    st.stop()
...
# 4) Convert dtypes & drop critical nulls (dotted names)
...
# 8) PO-level metrics (uses dotted column names)
...
if breaches.empty:
    ...
    # vendor statistics for the year (dotted column names used for counting)
    total_orders_year = po_year.shape[0]
    vendor_stats = (
        po_year.groupby("Supplier", dropna=False)
               .agg(Total_Orders=("P. O. No.", "count"),
                    OTIF_Failures=("OTIF", lambda x: int((x==0).sum())),
                    OTIF_Success=("OTIF", lambda x: int((x==1).sum())))
               .reset_index()
    )
    vendor_stats["Supplier"] = vendor_stats["Supplier"].fillna("Unknown Supplier")
    ...
    display_top10 = top10[["Supplier", "OTIF_Failures", "Vendor_OTIF_pct", "Total_Contribution_pct", "Total_Orders"]].copy()
    ...
    if reportlab_available:
        try:
            pdf_bytes = generate_failed_orders_pdf(breaches[["Supplier", "GNR Dt.", "P. O. No."]], vendor_stats, selected_year)
            st.download_button(
